	...
	$.extend(IkSelect.prototype, {
		...
		_makeOptionActive: function (index, shouldSync) {
			var $option = $(this.el.options[index]),
				...
				text = $option.text();
			if (img){
				text = '<img src="' + img + '" alt="" class="select-img"> ' + text;
			}
		...
		reset: function () {
			...
			var selectedOption = this.$el.find('option:selected'),
				img = selectedOption.data('img');
			var newVal = this.$el.val();
			if (img){
				newVal = '<img src="' + img + '" alt="" class="select-img"> ' + newVal;
			}
			this.$linkText.html(newVal);
		...
		hideDropdown: function () {
			if (b2.scrollClicked){ // ULMIX CUSTOM FIX
				b2.scrollClicked = false;
				return false;
			}
		...
		_generateOptionObject: function ($option) {
			...
			if (img){
				img = '<img src="' + img + '" alt="" class="select-img"> '
			}
			return {
				value: $option.val(),
				label: (img + $option.html()) || '&nbsp;',
