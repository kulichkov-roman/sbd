function JCSmartFilter(ajaxURL, form_id, manual, hide_props) {
	...
	if(typeof values.rz_all_elements != "undefined"){
		values.set_filter = 'y';
	}
	...
	if(this.ajaxURL.indexOf('/apply/') > -1 && this.ajaxURL.indexOf('/clear/') == -1){
		$('.rbs-filter-shows').show();
		$('.main-block .category').addClass('rbs-padding-filter');
	}
	...
	this.$form.on('click', '#del_filter', function (e) {
        ...
        $(smartFilter.form).find('.show .show-results').addClass('disabled');
        $(smartFilter.form).find('.jq-checkbox.js-formstyler').removeClass('checked');
		// hide filter results
		$('#modef, #flying-results-wrap', '.rbs-filter-shows').hide();
		$('.main-block .category').removeClass('rbs-padding-filter');

		// clear sliders
		$(smartFilter.form).find('[id^=slider_]').each(function () {
			var minInput = $(this).siblings('.range').find('input.range-input-lower');
				maxInput = $(this).siblings('.range').find('input.range-input-upper');
				min = minInput.data('range-min');
				max = maxInput.data('range-max');

			minInput.attr({placeholder: parseInt(min)});
			maxInput.attr({placeholder: parseInt(max)});
	...
	//if(this.manual) {
		$(document).on('click', '#flying-results-wrap .btn-show-results, #' + form_id + ' .show-results,.rbs-close-filter-btn', function (e) {
			e.preventDefault();
			//RZB2.ajax.CatalogSection.Start();
			RZB2.ajax.scrollPage($('.sort-n-view'), true);
			$('.filters-toggle.js-filters').trigger('click');
		});
	//}
...
JCSmartFilter.prototype.reload = function (input) {
	...
	var _this = $(input);		
	if(_this.hasClass('rbs-clr-label')){
		if(!_this.hasClass('rbs-clr-label-choosen'))
			_this.addClass('rbs-clr-label-choosen');
		else
			_this.removeClass('rbs-clr-label-choosen');
	}
...
JCSmartFilter.prototype.postHandler = function (result) {
	...
	if (!!result && !!result.ITEMS) {
				...
				if (arItem.DISPLAY_TYPE == 'R' || arItem.DISPLAY_TYPE == 'P') {
					...
					for (var i in arItem.VALUES) {
						...
						if (control && $(control).is('select')) {
                            ...
                            $(control.parentNode).closest('.list-check__item').toggleClass(className, (!!ar.DISABLED && !ar.CHECKED));
							...
							if (ar.hasOwnProperty('ELEMENT_COUNT'))
							{
								label = $('[data-role="count_'+ar.CONTROL_ID+'"]');

								if (label)
                                    label.find('.count').html(ar.ELEMENT_COUNT);
							}
							$brand = undefined;
						}
					}
		...
		if(result.FILTER_URL.indexOf('clear')>-1){
			$('.rbs-filter-shows').hide();
			$('.main-block .category').removeClass('rbs-padding-filter');
		} else {
			$('.rbs-filter-shows').show();
			$('.main-block .category').addClass('rbs-padding-filter');
	...
	if (YSFilterRemoveDisable) {
		...
		this.$form.find('.show .show-results').removeClass('disabled');
	}
...
function createSlider(params) {
	...
	var selectorSlider = '#' + params.SLIDER_ID;
	var selectorMin = '#' + params.INPUT_ID.MIN;
	var selectorMax = '#' + params.INPUT_ID.MAX;
    ...
    if ($(selectorSlider).length) {
        ...
        $(selectorMin).val($(selectorSlider).slider("values", 0));
        $(selectorMax).val($(selectorSlider).slider("values", 1));
        $(selectorMin).change(function () {
            $(selectorSlider).slider("values", 0, $(this).val());
        });
        $(selectorMax).change(function () {
            $(selectorSlider).slider("values", 1, $(this).val());
		});
		
		$(selectorSlider).data('range', { min: minPriceLimit, max: maxPriceLimit })
		setRangeSliderLimits($(selectorSlider), {left:minFilterLimit, right:maxFilterLimit});
    }
}
