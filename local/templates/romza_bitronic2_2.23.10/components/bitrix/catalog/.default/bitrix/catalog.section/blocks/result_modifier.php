...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as $index => &$arItem) {
       ...
       /*  echo '<pre>';
        print_r([$arItem['NAME'] => $arItem['CATALOG_QUANTITY']]);
        echo '</pre>'; */
        /* /ВСТАВИТЬ ПОСЛЕ ОБНОВЛЕНИЯ*/
        ...
        if ($arParams['SEARCH_PAGE'] != 'Y' && CRZBitronic2Settings::isPro() && CRZBitronic2CatalogUtils::checkAvPrFotoForElement($arItem, $arParams)) {
        ...
        ob_start();
        $arItem['STICKERS'] = $APPLICATION->IncludeComponent("yenisite:stickers", "sib_section", array(
            "ELEMENT" => $arItem,
