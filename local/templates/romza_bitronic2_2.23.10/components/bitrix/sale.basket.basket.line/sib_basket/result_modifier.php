...
if(!empty($arResult["CATEGORIES"]["READY"]))
{
	...
	foreach($arResult["CATEGORIES"]["READY"] as $key=>$item){
		...
		$arResult["CATEGORIES"]["READY"][$key]['MAIN_PHOTO'] = CRZBitronic2CatalogUtils::getElementPictureById($item["PRODUCT_ID"], 3);
		$arResult["CATEGORIES"]["READY"][$key]['MAIN_PHOTO_JPG'] = CRZBitronic2CatalogUtils::getElementPictureById($item["PRODUCT_ID"], 43);
