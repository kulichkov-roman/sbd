...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		initializePrimaryFields: function()
		{
			...
			/*RBS_CUSTOM_START*/
			this.domain = 'sibdroid.ru';
			this.coockieTime = 60 * 60 * 24 * 30;
			this.customCache = {};
			/*RBS_CUSTOM_END*/
		...
		init: function(parameters)
		{
			...
			/*RBS_CUSTOM_START*/
			this.deliveryType = BX.getCookie('rbs_delivery_type') ? BX.getCookie('rbs_delivery_type') : '';
			this.firstDeliveryId = {self: 0, courier: 0};
			this.currentStep = BX.getCookie('rbs_current_step') ? BX.getCookie('rbs_current_step') : '';
			/*RBS_CUSTOM_END*/
			...
			this.sibIco = this.templateFolder + "/images/sibico.jpg";
			...
			/*RBS_CUSTOM_START*/
			this.eventPararmsRetarget = {};
			this.setVkDataLayer();
			this.customHandlers();
			this.currentStepActive();
			/*RBS_CUSTOM_END*/
		},

		/*RBS_CUSTOM_START*/
		currentStepActive: function()
		{
			if(this.currentStep != ''){
				var currStep = this.currentStep;
				while(currStep != this.activeSectionId){
					if(this.clickNextAction({target: document.querySelector('.bx-soa-more-btn a:last-child')}) == 'stopNextPage'){
						return false;
					}
				}

				if(this.currentStep == this.deliveryBlockNode.id){
					if(this.deliveryType == 'self'){
						if(this.isMobile && this.firstLoad){
							this.sendRequest();
						}
					}
				}
		...
		customHandlers: function()
		{
			var _this = this;

			//переключение этапов в шапке
			$('[data-section-bind]').off().on('click', function(){
				var lastStepActive = $('[data-section-bind="' + _this.activeSectionId + '"]'),
					clickedStepId = $(this).data('section-bind');
				
				if(lastStepActive.length){					
					if(lastStepActive.data('next-section-bind') == clickedStepId) {
						_this.clickNextAction({
							target: document.querySelector('.bx-soa-more-btn a:last-child')
						});
					} else if(lastStepActive.data('prev-section-bind') == clickedStepId) {
						_this.clickPrevAction({
							target: document.querySelector('.bx-soa-more-btn a:first-child')
						});
					}
				}
			});
			
			//привязка свойств к кукам
			$('#soa-property-1, #soa-property-2, #soa-property-3')
				.off()
				.on('keyup', function(){
					BX.setCookie($(this).attr('name'), $(this).val(), {domain:_this.domain,expires:this.coockieTime});
				})
				.each(function(){
					var value = _this.params.USER_INFO[$(this).attr('name')].trim() || BX.getCookie($(this).attr('name'));
					$(this).val(value);
				});

			//маска телефона + валидация
			maskPhoneInit($('#soa-property-3'));
			$('#soa-property-3').on('change', function(){
				var parentNode = $(this).closest('.soa-property-container');
				if ($(this).val().trim() == ''){					
					parentNode.addClass('input-error');
					parentNode.removeClass('input-ok');
				} else {
					parentNode.removeClass('input-error');
					parentNode.addClass('input-ok');
				}
			});

			this.selectDeliveryTypeItem();

			if($('.address-input-group').length){
			...
			//выбор пункта самовывоза едост
			/* if($('#rbs-edost-select-point').length){
				$('#rbs-edost-select-point').off().on('change', function(){
					if($(this).val() > 0){
						BX.setCookie('edostActiveId', $(this).val(), {expires: 86400});
						window.edostActiveId = $(this).val();
						$('#edost_office').val($(this).val());
						_this.sendRequest();
					}
				});
			} */

			if($('.rbs-edost-map').length){
				$('.rbs-edost-map').on('touchmove', function(event){
					if(event.targetTouches.length == 1){
						if(!$(this).hasClass('map-visible'))
							$(this).addClass('map-visible');
					} else {
						if($(this).hasClass('map-visible'))
							$(this).removeClass('map-visible');
					}					
				});
				$('.rbs-edost-map').on('touchend', function(){
					if($(this).hasClass('map-visible'))
						$(this).removeClass('map-visible');
				});
			}
			...
			if($('.close-btn').length){
				$('.close-btn').off().on('click', function(){
					$(this).closest('.right-delivery-block').hide();
				});
			}

			if($('.rbs-close-right-block').length){
				$('.rbs-close-right-block').off().on('click', function(){
					$(this).closest('.right-delivery-block').hide();
					if('edost_office_c' in window){
						edost_office_c.map.setZoom(10);
					}
				});
			}
			...
			if(this.rootCity){
				if($('.rbs-edost-map').length){
					$('.rbs-edost-map').addClass('rbs-root-city');
				}
			}
		},

		implodeAddress: function()
		{
			if(this.deliveryType == 'courier'){
				var addr = '';
				$('.address-input-group input').each(function(){
					if($(this).val()){
						addr += $(this).attr('placeholder') + ': ' + $(this).val() + ' ';
					}					
				});
				if(addr != ''){
					$('#soa-property-7').text(addr).val(addr);
				}
			}
		},

		edostMapCreate: function(params)
		{
			if($('.pp-block-active #edost_get_office_span').length){
				if(params.isAjax === false){
					$('.pp-block-active #edost_get_office_span').trigger('click');
				} else {
					if('edost_office_c' in window && 'edost_office_create_c' in window){
						window.edost_office_c = new edost_office_create_c('edost_office_c');
						$('.pp-block-active #edost_get_office_span').trigger('click');
					}
				}				
			}			
		...
		cityPopup: function(targetClass, isMobile)
		{
			var _this = $('.' + targetClass);
			if(_this.length){
				if(!_this.hasClass('loaded-city')){
                    $.ajax({
                        url: '/include_areas/sib/header/vregions_hide.php',
                        method: 'post',
                        data: {'ajax-city-hide': 'Y', 'request-uri': window.location.pathname,'is-mobile': isMobile},
                        success: function(data){
                            $('.order-delivery-city-choose .city-hide').html($(data).html());                     
							_this.addClass('loaded-city');
							$('.order-delivery-city-choose .city-hide').show();
							$(".rbs-soa-mask").show();
							$('.order-delivery-city-choose .city-hide').find(".city-hide__button").on('click', function(){
								setTimeout(()=>{$(".rbs-soa-mask").hide();});
							});
                        }
                    });
                } else {
			...
			else
			{
				BX.ajax({
					...
					onsuccess: BX.delegate(function(result) {
						...
						/*RBS_CUSTOM_START*/
						this.customHandlers();
						this.edostMapCreate({isAjax: true});
						/*RBS_CUSTOM_END*/

						this.endLoader();

						if(this.needUpdateEdost && !!window.edostActiveId){
							$('#edost_office').val(window.edostActiveId);
							this.sendRequest();
						}
			...
			if (result && result.order)
			{
				...
				if (result.REDIRECT_URL)
				{
					...
					if(typeof VK !== 'undefined'){
						VK.Retargeting.ProductEvent(PRICE_LIST_ID, "purchase", this.eventPararmsRetarget);
					}
		...
		initPagination: function()
		{
			...
			/*RBS_CUSTOM_START*/
			var checkedId = false, deliveryId, current;
			if(!('RESULT_DELIVERY' in this.customCache)){
				if(this.result.DELIVERY){
					this.customCache.RESULT_DELIVERY = this.result.DELIVERY;
				} else {
					this.customCache.RESULT_DELIVERY = false;
				}				
			} else {
				if(this.customCache.RESULT_DELIVERY){
					var checkedId = false;
					for(deliveryId in this.result.DELIVERY){
						if('CHECKED' in this.result.DELIVERY[deliveryId] && this.result.DELIVERY[deliveryId].CHECKED === 'Y'){
							checkedId = deliveryId;
							break;
						}
					}
					for(current in this.customCache.RESULT_DELIVERY){
						if(current == checkedId){
							this.customCache.RESULT_DELIVERY[current] = this.result.DELIVERY[current];
						} else {
							this.customCache.RESULT_DELIVERY[current].CHECKED = 'N';
						}
					}

					this.result.DELIVERY = this.customCache.RESULT_DELIVERY;
				}
			}
			/*RBS_CUSTOM_END*/
			if (this.result.DELIVERY)
			...
			{
				/*RBS_CUSTOM_START*/
				if(typeof this.result.PAY_SYSTEM.length == 'undefined'){
					var paySystemArray = [], paySys;
					for(paySys in this.result.PAY_SYSTEM){
						paySystemArray.push(this.result.PAY_SYSTEM[paySys]);
					}
					this.result.PAY_SYSTEM = paySystemArray;
				}
				/*RBS_CUSTOM_END*/
		...
		clickNextAction: function(event)
		{
			...
			if(actionSection.getAttribute('id') == this.propsBlockNode.getAttribute('id')){
				if(this.isValidPropertiesBlock().length > 0){
					return 'stopNextPage';
				}
			}
			if(actionSection.getAttribute('id') == this.deliveryBlockNode.getAttribute('id')){
				if(!this.deliveryType){
					return 'stopNextPage';
				}
				if(this.deliveryType == 'courier'){
					this.implodeAddress();
					if($('[name="d_street"]').val().trim() == ''){
						$('.left-delivery-block.courier-block').prepend($(`
							<div class="rbs-error-street">
								Введите улицу для доставки
							</div>
						`));
						return 'stopNextPage';
					} else if($('.rbs-error-street').length){
						$('.rbs-error-street').remove();
					}
				} else if(this.deliveryType == 'self'){
					if(this.needUpdateEdost && window.edostActiveId){
						this.sendRequest();
					}
				}
			...
			/*RBS_CUSTOM_START*/
			var stepActivate = $('[data-section-bind="' + section.next.getAttribute('id') + '"]');
			if(stepActivate.length){
				$('[data-section-bind]').removeClass('current-step');
				stepActivate.addClass('step-active');
				stepActivate.addClass('current-step');
			}
			this.currentStep = section.next.getAttribute('id');
			BX.setCookie('rbs_current_step', this.currentStep, {domain:this.domain});
			/*RBS_CUSTOM_END*/
		...
		clickPrevAction: function(event)
		{
			...
			/*RBS_CUSTOM_START*/
			var stepActivate = $('[data-section-bind="' + actionSection.getAttribute('id') + '"]'),
				currentStep = $('[data-section-bind="' + section.next.getAttribute('id') + '"]');
			if(stepActivate.length){
				stepActivate.removeClass('step-active');
				$('[data-section-bind]').removeClass('current-step');
		...
		shouldSkipSection: function(section)
		{
			...
			/*RBS_CUSTOM_START*/
			if(section.id == this.regionBlockNode.getAttribute('id')){
				return true;
			}
			if(section.id == this.basketBlockNode.getAttribute('id')){
				return true;
			}
			/*RBS_CUSTOM_END*/
		...
		getBlockFooter: function(node)
		{
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'pull-right button'},
		...
		createPaySystemItem: function(item)
		{
			...
			{
				title = BX.create('DIV', {
					props: {className: 'pp-block-title'},					
					children: [
						BX.create('DIV', {props:{className: ''}}),
						BX.create('SPAN', {text: item.NAME})
					]
				});
				
				var description = BX.create('DIV', {props: {className: 'pp-block-description'}, html: item.DESCRIPTION});
				var ppBlock = BX.create('DIV', {
					props: {className: 'pp-block'},
			...
			if(item.ID == 27 && !this.params.IS_ADMIN){
				hideClass = ' hide';
			}

			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-6 col-sm-6 col-xs-12' + hideClass},
				children: [label, ppBlock],
				events: {
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			if (this.initialized.delivery)
			{
				...
				/*RBS_CUSTOM_START*/
				this.edostMapCreate({isAjax: false});
				/*RBS_CUSTOM_END*/
			}
			else
			{
				...
				/*RBS_CUSTOM_START*/
				this.getDeliveryOptionsContainer(deliveryNode);
				/*RBS_CUSTOM_END*/
		...
		getDeliveryOptionsContainer: function(deliveryNode)
		{
			if (!this.result.DELIVERY || this.result.DELIVERY.length <= 0)
				return;

			/*Город*/
			var currentCity = BX.create('DIV', {props:{className:'order-delivery-city-choose'}, html: `
				<div class="city city_delivery">
					${this.params.CITY_NAME}
					<div class="city-hide"></div>
				</div>
			`}),
				cityChoose = BX.create('DIV', {props: {className: 'col-sm-12'}, children:[
					BX.create('DIV', {props:{className:'order-delivery-town'}, children:[
						BX.create('SPAN', {text: 'Доставка в '}),
						currentCity
					]})
				]});

			BX.unbindAll(currentCity);
			BX.bind(currentCity, 'click', BX.delegate(function(e){
				this.cityPopup('order-delivery-city-choose', this.isMobile);
			}, this));

			/*Самовывоз*/
			var deliveryTypeChoose = BX.create('DIV', {props:{className:'col-sm-12'}, children:[
				BX.create('DIV', {props:{className:'rbs-delivery-type row'}, children: this.getDeliveryTypes()})
			]});
			
			var deliveryOpyionsContainer = BX.create('DIV',   
				{
			...
			deliveryNode.appendChild(deliveryOpyionsContainer);
		},

		getDeliveryTypes: function()
		{
			var activeClassSelf = this.deliveryType == 'self' ? 'rbs-delivery-type__active' : '';
			var activeClassCour = this.deliveryType == 'courier' ? 'rbs-delivery-type__active' : '';
			var deliveryTypes = [
				BX.create(
					'DIV',
					{
						...
						children: [
							BX.create(
								'DIV',
								{
									props: {className: 'rbs-delivery-type__type type_self ' + activeClassSelf},
									children: [
										BX.create('INPUT',{props:{type: 'radio',id: 'delivery_type_self',value: 'self',className: 'hide', checked: activeClassSelf != ''}}),
										BX.create('DIV',{props:{className: 'rbs-delivery-type__head'},text: 'Самовывоз'}),
										BX.create('DIV',{props:{className: 'rbs-delivery-type__descr'},text: 'В удобном месте'})
									],
									events:{
										click: BX.proxy(function(e){
											this.selectDeliveryType('self');
										}, this)
									}
								}
							)
					...
					'DIV',
					{
						...
						children: [
							BX.create(
								'DIV',
								{
									props: {className: 'rbs-delivery-type__type type_courier ' + activeClassCour},
									children: [
										BX.create('INPUT',{props:{type: 'radio',id: 'delivery_type_self',value: 'courier',className: 'hide', checked: activeClassCour != ''}}),
										BX.create('DIV',{props:{className: 'rbs-delivery-type__head'},text: 'Курьер'}),
										BX.create('DIV',{props:{className: 'rbs-delivery-type__descr'},text: 'Домой или в офис'})
									],
									events:{
										click: BX.proxy(function(e){
											this.selectDeliveryType('courier');
										}, this)
									}
								}
							)
		...
		selectDeliveryType: function(deliveryType)
		{
			if(deliveryType){
				...
				$('#delivery_type_self').val(this.deliveryType);
				$('.rbs-delivery-type__type').removeClass('rbs-delivery-type__active');
				$('.type_' + this.deliveryType).addClass('rbs-delivery-type__active');
				
				BX.setCookie('rbs_delivery_type', this.deliveryType, {expires: this.coockieTime, domain: this.domain});

				this.selectDeliveryTypeItem()
			}
		},

		selectDeliveryTypeItem: function()
		{			
			if(this.deliveryType != ''){
				for(var k in this.deliveryPagination.currentPage){
					var current = this.deliveryPagination.currentPage[k];
					if(current.CHECKED == 'Y'){
						if(this.deliveryType != current.DELIVERY_TYPE){
							$('[name="DELIVERY_ID"]').val(this.firstDeliveryId[this.deliveryType]);
							this.sendRequest();
						} else {
							$('.item-type__' + this.deliveryType).not('.only-for-admin').removeClass('hide');
						}
					}
				}
		...
		createDeliveryItem: function(item)
		{
			...
			var checked = item.CHECKED == 'Y',
				...
				calssNameItem = 'bx-soa-pp-company col-xs-12',
				labelNodes = [
					BX.create('INPUT', {
						props: {
			...
			item.isSibdroidDelivery = 'DESCRIPTION' in item && item.DESCRIPTION.toLowerCase() === 'sibdroid';
			if(item.isSibdroidDelivery){
				item.extraParams = JSON.parse(item.CALCULATE_DESCRIPTION);
			}
			
			if(this.params.DELIVERY_TYPE_ARRAY[item.ID]){
				item.DELIVERY_TYPE = this.params.DELIVERY_TYPE_ARRAY[item.ID];
			} else {
				item.DELIVERY_TYPE = 'none';
			}

			if(this.firstDeliveryId[item.DELIVERY_TYPE] <= 0){
				this.firstDeliveryId[item.DELIVERY_TYPE] = item.ID;
			}

			if(item.DELIVERY_TYPE != this.deliveryType){
				calssNameItem += ' hide';
			}
			
			calssNameItem += ' item-type__' + item.DELIVERY_TYPE;
			...
			title = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {props:{className: ''}}),
					BX.create('SPAN', {text: item.OWN_NAME})
				]
			});

			//Построение блока	
			
				this.modifyCustomItem(item);
				var rightBlockHtml = this.getShortInfoOfDelivery(item) + item.rightBlockAfterHtml,  //короткая инфа о блоке
					leftBlockHtml = ``, 
					childPpBlock = [],
					ppBlockClass = '',
					leftClassBlock = '';
				
				item.isEdostItem = false;
				if(item.DESCRIPTION){
					item.isEdostItem = item.NAME.indexOf('СДЭК') != -1 || item.NAME.indexOf('DPD') != -1 || item.NAME.indexOf('boxberry') != -1;
				}
				
				//построение активного блока
				if(checked){	
															
					ppBlockClass = 'pp-block-active';
					if(item.DELIVERY_TYPE == 'self'){
						if(item.isEdostItem){
							item.DESCRIPTION = item.DESCRIPTION.split('edost_office.window').join('edost_office_c.window');
							if(item.DESCRIPTION.indexOf('edost_office_c.window') == -1){
								item.DESCRIPTION += `<span id="edost_get_office_span" onclick="edost_office_c.window('office')"></span>`;	
							}
						} else {
							if(item.isSibdroidDelivery){

							} else {
								item.DESCRIPTION += `<span id="edost_get_office_span" onclick="edost_office_c.window('shop')"></span>`;		
							}
												
						}
						leftBlockHtml = `<div style="display:none">${item.DESCRIPTION}</div><div class="rbs-edost-map"></div>`;

						if(item.isSibdroidDelivery){
							leftBlockHtml = `<span id="edost_get_office_span" onclick="edost_office_c.window('shop')"></span><div style="display:none">${item.CUSTOM_DESCRIPTION}</div><div class="rbs-edost-map"></div>`;
						}

					} else if(item.DELIVERY_TYPE == 'courier') {
						leftBlockHtml = `
							<div class="address-input-group">
								<input type="text" name="d_street" placeholder="Улица" class="w-30">
								<input type="text" name="d_home" placeholder="Дом" class="w-20">
								<input type="text" name="d_room" placeholder="Квартира" class="w-20">
								<input type="text" name="d_entrance" placeholder="Подъезд" class="w-20">
								<input type="text" name="d_comment" placeholder="Комментарий к адресу" class="w-100">
							</div>
						`;
						leftClassBlock = 'courier-block';
					}
					
					rightBlockHtml = this.getFullInfoOfDelivery(item) + rightBlockHtml;
					if(this.moreDeliveryBtn){
						rightBlockHtml += `<div class="button rbs-close-right-block">Изменить пункт самовывоза</div>`;
					}
					
					childPpBlock.push(title);
					childPpBlock.push(
						BX.create('DIV', {
							props: {className: 'pp-block-description'},
							children: [
								BX.create('DIV', {
									props:{className: 'left-delivery-block ' + leftClassBlock},
									html: leftBlockHtml
								}),
								BX.create('DIV', {
									props:{className: 'right-delivery-block'},
									children: [
										BX.create('SPAN', {
					...
					ppBlockClass = 'pp-block-in-active';

					childPpBlock.push(title);
					childPpBlock.push(
						BX.create('DIV', {
							props:{className: 'right-title-block right-delivery-block'},
							children: [
								BX.create('DIV', {
									html: rightBlockHtml
								})
				...
				var ppBlock = BX.create('DIV', {
					props: {className: 'pp-block ' + ppBlockClass},
					children: childPpBlock
				});
			//Построение блока	
			//}

			itemNode = BX.create('DIV', {
				props: {className: calssNameItem},
				children: [label, ppBlock],
			...
			modifyCustomItem(item)
			{
				item.rightBlockAfterHtml = ``;

				//модификация кастомной службы
				if(item.isSibdroidDelivery){
					//item.rightBlockAfterHtml = `<div><b>Можно забрать:</b> через 10 минут</div>`;
					var gps = [];
					item.extraParams.GPS.trim().split(',').forEach(function(item){
						gps.push(item.trim());
					});
					item.extraParams.GPS = '' + gps[1] + ',' + gps[0];
					
					item.CUSTOM_DESCRIPTION = `
						<input 
							id="edost_office_data"
							autocomplete="off"
							value='{
								"ico_path": "/bitrix/images/delivery_edost_img",
								"point": [{"company_id": "s1", "data": ["14546A10524|${item.extraParams.NAME}|${item.extraParams.ADR}|${item.extraParams.SHEDULE.split('<br>').join(',')}|${item.extraParams.GPS}|||"]}],
								"tariff": ["69_348|Самовывоз||35|0|${item.PRICE} руб.|||||||s1|shop|"]}' 
								type="hidden"
						>
						<input type="hidden" value="" id="edost_office" name="edost_office">
						<input type="hidden" value="7" id="edost_address_input">
						<span id="edost_get_office_span" onclick="edost_office_c.window('shop')"></span>`;
						
				}
				
				//модификация новосиба самовывоз
				if(item.ID == 348){
					item.rightBlockAfterHtml = `<div><b>Можно забрать:</b> через 10 минут</div>`;
				}
				if(item.ID == 340){
					item.day = `<div style="white-space:normal">Сегодня при заказе до 17:00</div>`;
					if(this.params.CITY_NAME == 'Москва'){
						item.day = `<div style="white-space:normal">Сегодня при заказе до 15:00</div>`;
					}
				}
				
				//кастомная служба для городов с нерабочим самовывозом
				if(item.ID == 422){
					if(this.params.IS_MSK_REGION){
						item.CITY_NAME = 'Москва';
						item.DESCRIPTION = `<div class="edost"><span class="edost_address_head_n" style="font-size: 14px; color: #888; display: block;">Магазин:</span> <b style="font-size: 14px;">Багратионовский проезд 7 к 3 (ТК Горбушкин двор, м. Багратионовская)</b> <a class="edost_link" class="edost_address_map_n" style=" display: block;" href="http://www.edost.ru/office.php?c=14546A10524" target="_blank">показать на карте</a></div><input id="edost_office_data" autocomplete="off" value='{"ico_path": "/bitrix/images/delivery_edost_img", "point": [{"company_id": "s1", "data": ["14546A10524|ТК Горбушкин двор|Багратионовский проезд 7 к 3|пн-вс с 10:00 до 21:00|37.504043,55.741490|3|Багратионовская|"]}], "tariff": ["69_348|Самовывоз||35|0|0|||||||s1|shop|"]}' type="hidden"><input type="hidden" value="" id="edost_office" name="edost_office"><input type="hidden" value="7" id="edost_address_input"><br>г. Москва, Багратионовский проезд 7 корпус 3 ТК Горбушкин Двор пав. h2-003<br>Режим работы: пн-вс с 10:00 до 21:00<span id="edost_get_office_span" onclick="edost_office_c.window('shop')"></span>`;
						item.office_data = {
							'14546A10524': {
								address: "Багратионовский проезд 7 к 3",
								address2: "h2-003",
								address_full: "Багратионовский проезд 7 к 3, h2-003",
								code: false,
								gps: "37.504043,55.741490",
								id: "14546A10524",
						...
						item.CITY_NAME = 'Новосибирск';
						item.DESCRIPTION = `<div class="edost"><span class="edost_address_head_n" style="font-size: 14px; color: #888; display: block;">Магазин:</span> <b style="font-size: 14px;">ул. Новогодняя 17 (м. Площадь Маркса)</b> <a class="edost_link" class="edost_address_map_n" style=" display: block;" href="http://www.edost.ru/office.php?c=5870A7559" target="_blank">показать на карте</a></div><input id="edost_office_data" autocomplete="off" value='{"ico_path": "/bitrix/images/delivery_edost_img", "point": [{"company_id": "s1", "data": ["5870A7559||ул. Новогодняя 17|пн.-вс. с 9:00 до 21:00|82.903802,54.983349|3|Площадь Маркса|"]}], "tariff": ["69_348|Самовывоз||35|0|0|||||||s1|shop|"]}' type="hidden"><input type="hidden" value="" id="edost_office" name="edost_office"><input type="hidden" value="7" id="edost_address_input"><br>Самовывоз<span id="edost_get_office_span" onclick="edost_office_c.window('shop')"></span>`
						item.office_data = {
							'5870A7559': {
								address: "ул. Новогодняя 17",
								address2: false,
								address_full: "ул. Новогодняя 17",
								code: false,
								gps: "82.903802,54.983349",
								id: "5870A7559",
			...
			getShortInfoOfDelivery: function(item)
			{
				var rightBlockHtml = ``;

				if(item.PRICE > 0){
					rightBlockHtml += `<div><b>Стоимость:</b> <span class="current-price">${item.PRICE} </span></div>`;
				} else {
					rightBlockHtml += `<div><b>Стоимость:</b> бесплатно</div>`;
				}
				
				if(item.day){
					rightBlockHtml += `<div><b>Срок доставки:</b> ${item.day}</div>`;
				} else if(item.DELIVERY_TYPE == 'courier' && item.DESCRIPTION && !item.isSibdroidDelivery){
					rightBlockHtml += `<div><b>Срок доставки:</b> ${item.DESCRIPTION}</div>`;
				}

				if(item.isSibdroidDelivery){
					rightBlockHtml += `<div><b>Срок доставки:</b> ${item.PERIOD_TEXT}</div>`;
				}

				return rightBlockHtml;
			},

			getFullInfoOfDelivery: function(item)
			{
				var selectedItem = 0, rightBlockBefore = ``, cntOffice = 0, j, selectFirstItem;
				this.cityDeliveryName = item.CITY_NAME || this.params.CITY_NAME;
				var cityHtml = `<div>г. ${this.cityDeliveryName}</div>`;
				this.needUpdateEdost = false;
				this.moreDeliveryBtn = false;
				
				if(item.DELIVERY_TYPE == 'self'){
					rightBlockBefore += `
						<div class="rbs-choose-point-self"><b>Выбранный пункт самовывоза:</b></div>
					`;

					//this.sibIco

					if(item.isEdostItem){
						if('office_data' in item){
							
							for(j in item.office_data){
								cntOffice++;
							}

							if(cntOffice > 1){								
								//rightBlockBefore += '<div>';
									//rightBlockBefore += '<select id="rbs-edost-select-point">';
									//	rightBlockBefore += `<option value=0>Не выбрано</option>`;
										for(j in item.office_data){
											if(window.edostActiveId && j == window.edostActiveId){
												//rightBlockBefore += `<option value=${j} selected>${item.office_data[j].address}</option>`;
												selectedItem = j;
											} else {
												//rightBlockBefore += `<option value=${j}>${item.office_data[j].address}</option>`;
											}
									...
									//rightBlockBefore += '</select>';
								//rightBlockBefore += '</div>';
							
								if(selectedItem > 0){
									rightBlockBefore += `<div class="rbs-self-address-div"><i style="background-image:url(${this.sibIco})"></i>${cityHtml}<span class="rbs-self-address">${item.office_data[selectedItem].address}</span></div>`;
								} else {
									if(selectFirstItem){
										BX.setCookie('edostActiveId', selectFirstItem, {expires: this.coockieTime});
										window.edostActiveId = selectFirstItem;
										$('#edost_office').val(selectFirstItem);
										this.needUpdateEdost = true;
									}
								}

								this.moreDeliveryBtn = true;
							} else {
								for(j in item.office_data){
									window.edostActiveId = selectedItem = j;
									rightBlockBefore += `<div class="rbs-self-address-div"><i style="background-image:url(${this.sibIco})"></i>${cityHtml}<span class="rbs-self-address">${item.office_data[j].address}</span></div>`;
								}								
							}							
						}
						if(selectedItem > 0){
							if(item.office_data[selectedItem].schedule){
								var shedule = '';
								item.office_data[selectedItem].schedule.split(',').forEach(function(item){
									shedule += `<div>${item}</div>`;
								});
								rightBlockBefore += `<div class="rbs-shedule"><b>Режим работы:</b> <div>${shedule}</div></div>`;
							}						
						}
					} else {
						var point = false;
						...
						if(point){
							if(point.address){
								rightBlockBefore += `<div class="rbs-self-address-div"><i style="background-image:url(${this.sibIco})"></i>${cityHtml}<span class="rbs-self-address">${point.address}</span></div>`;
							}
							/* if(point.metro){
								rightBlockBefore += `<div><b>Метро:</b> ${point.metro}</div>`;
							} */
							if(point.schedule){
								rightBlockBefore += `<div class="rbs-shedule"><b>Режим работы:</b> <div>${point.schedule}</div></div>`;
							}
						}
						
						if(item.isSibdroidDelivery){

							rightBlockBefore += `<div class="rbs-self-address-div"><i style="background-image:url(${this.sibIco})"></i>${cityHtml}<span class="rbs-self-address">${item.extraParams.ADR}</span></div>`;
								

							var sheduleCustom = '';
							item.extraParams.SHEDULE.split('<br>').forEach(function(item){
								sheduleCustom += `<div>${item}</div>`;
							})
							rightBlockBefore += `<div class="rbs-shedule"><b>Режим работы:</b> <div>${sheduleCustom}</div></div>`;
						}
					}
				}
		...
		selectDelivery: function(event)
		{
			...
			if(
				event.target.className == 'ymaps-2-1-74-balloon__close-button' 	||
				event.target.className == 'ymaps-image-with-content-content' 	||
				event.target.className == 'ymaps-image-with-content' 			||
				event.target.className == 'ymaps-b-balloon__close'
			){
				return BX.PreventDefault(event);
			}

			var isSelfSelected = false;
			if(event.target.className == 'edost_button_right'){
				isSelfSelected = true;
			}
			...
			if (!isSelfSelected && selectedSection)
			{
					...
					while (property = propsIterator())
					{
						for (i = 0; i < props.length; i++)
							if (props[i] == property.getId() && property.getSettings()['IS_ZIP'] != 'Y')
								this.getPropertyRowNode(property, node, false, true);
					}
				}
			}
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-6 bx-soa-customer'}}),
				propsItemsAfterContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer for-checkbox'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			if (!propsItemsContainer)
				propsItemsContainer = this.propsBlockNode.querySelector('.col-sm-6.bx-soa-customer');
			/*RBS_CUSTOM_END*/
			if (!propsItemsAfterContainer)
				propsItemsAfterContainer = this.propsBlockNode.querySelector('.col-sm-6.bx-soa-customer.for-checkbox');
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					...
					this.getPropertyRowNode(property, propsItemsContainer, propsItemsAfterContainer, false);
				}
			}
			...
			if(!this.result.IS_AUTHORIZED){
				var popup = this.isMobile ? '#popup-login-mobile' : '#popup-login';
				var authPropBlock = BX.create('DIV', {
					props: {className: 'col-sm-6 bx-soa-auth-block'},
					html: `<div class="warn">
					<p><strong>Уже регистрировались?</strong></p>
					<p><a class="warn__link js-go-tab-1" data-fancybox="" href="${popup}">Авторизуйтесь</a> для отслеживания статуса заказа в "Личном кабинете", накопления бонусов и автоматического заполнения форм</p>
				</div>`
				});
				if (!authPropBlock)
					authPropBlock = this.propsBlockNode.querySelector('.col-sm-6.bx-soa-auth-block');
			
				propsNode.appendChild(authPropBlock);
			}

			propsNode.appendChild(propsItemsAfterContainer);
			/*RBS_CUSTOM_END*/
		},

		getPropertyRowNode: function(property, propsItemsContainer, propsItemsAfterContainer, disabled)
		{
			...
			switch (propertyType)
			{
				...
				case 'Y/N':
					...
					if(propsItemsAfterContainer){
						propsItemsAfterContainer.appendChild(propsItemNode);
						return;
					}
					break;
				case 'NUMBER':
		...
		insertYNProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				var spanDescr = BX.create('SPAN', {html: property.getDescription()});	
				property.setValue('Y');
				property.appendTo(spanDescr);
				BX.bind(spanDescr, 'click', BX.delegate(function(){
					var checkbox = BX.findChild(spanDescr, {tag:'input', attribute:{type:'checkbox'}}),
						parentNode = BX.findParent(spanDescr, {class:'checkbox-custom'});
					if(checkbox.getAttribute('value') == 'Y'){
						checkbox.setAttribute('value', 'N');
						BX.removeClass(parentNode, 'active');
					} else {
						checkbox.setAttribute('value', 'Y');
						BX.addClass(parentNode, 'active');
					}					
				}, this));

				propContainer = BX.create('DIV', {props: {className: 'soa-property-container checkbox-custom active'}, children: [spanDescr]});
				/*RBS_CUSTOM_END*/				
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
				this.bindValidation(property.getId(), propContainer);
		...
		isValidProperty: function(data, fieldName)
		{
			...
			for (i = 0; i < data.inputs.length; i++)
			{
				/*RBS_CUSTOM_START*/
				if(
					data.inputs[i].getAttribute('name') == 'ORDER_PROP_21' 	&&
					data.inputs[i].getAttribute('type') == 'checkbox' 		&&
					data.inputs[i].getAttribute('value') == 'N'
				){
					inputErrors = ['Обязательное поле'];
				} else {
		...
		showValidationResult: function(inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {tagName: 'DIV', className: 'soa-property-container'}),
				label = formGroup && formGroup.querySelector('label'),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'soa-property-container'});
				if (errors[i] && errors[i].length){
					BX.addClass(inputDiv, 'input-error');
					BX.removeClass(inputDiv, 'input-ok');
				}	
				else{
					BX.addClass(inputDiv, 'input-ok');
					BX.removeClass(inputDiv, 'input-error');
				}
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = document.querySelector('.comment-order-wrap');
			if(!propsCommentContainer){
				propsCommentContainer = BX.create('DIV', {props: {className: 'comment-order-wrap'}});
				label = BX.create('SPAN', {
					attrs: {for: 'orderDescription'},
					props: {className: 'total__link'},
					events: {
						click: BX.proxy(function(){
							var commentBlock = $('.comment-order');
							if(commentBlock.length){
								commentBlock.toggle();
							}
						}, this)
					},
					html: 'Комментарий к заказу'
				});
				...
				propsCommentContainer.appendChild(label);
				propsCommentContainer.appendChild(input);

				/*RBS_CUSTOM_START*/
				BX.addCustomEvent('onAfterTotalBlockEdit', BX.delegate(function(e){
					//if(!document.querySelector('.comment-order-wrap'))
						this.totalInfoBlockNode.appendChild(propsCommentContainer);
				}, this));
				/*RBS_CUSTOM_END*/
			}
		},

		setVkDataLayer: function()
		{
			var eventParams = {}, products = [], product = {}, sum = 0;
			for (var j in this.result.GRID.ROWS){
				if (this.result.GRID.ROWS.hasOwnProperty(j)){
					if(this.result.GRID.ROWS[j].data.PRODUCT_ID in this.params.SECTION_IDS){
						product = {
							id: this.result.GRID.ROWS[j].data.PRODUCT_ID,
							group_id: this.params.SECTION_IDS[this.result.GRID.ROWS[j].data.PRODUCT_ID],
							price: parseInt(this.result.GRID.ROWS[j].data.SUM_NUM),
							price_old: parseInt(this.result.GRID.ROWS[j].data.SUM_BASE)
						};
						sum += product.price;
						if(product.price_old <= 0 || product.price >= product.price_old){
							delete product.price_old;
						}
			...
			this.eventPararmsRetarget = eventParams;
			setTimeout(function(){
		...
		editTotalBlock: function()
		{
			...
			/*RBS_CUSTOM_START*/
			this.totalInfoBlockNode.appendChild(BX.create('DIV', {props:{className:'total__heading'}, text: 'Ваш заказ'}));
				...
				if (this.result.GRID.ROWS.hasOwnProperty(j)){
					var innerItemHtml = `
						<div class="total-item">
							<div class="total-item-img"><img src="${this.result.GRID.ROWS[j].data.PREVIEW_PICTURE_SRC || this.result.GRID.ROWS[j].data.DETAIL_PICTURE_SRC}"></div>
							<div class="total-item-name">${this.result.GRID.ROWS[j].data.NAME}</div>
							<div class="total-item-price">
								<div class="total-item-price-item">${this.result.GRID.ROWS[j].data.QUANTITY}шт x ${this.result.GRID.ROWS[j].data.PRICE_FORMATED}</div>
								<div class="total-item-price-full">${this.result.GRID.ROWS[j].data.SUM}</div>
							</div>
						</div> 
					`;
					
					items.push(BX.create(
						'LI',
						{
							props: {className:'order-list__item'},
							html: innerItemHtml
						}
					))
				}
			}
			this.totalInfoBlockNode.appendChild(BX.create('UL', {props:{className:'order-list'}, children: items}));

			var activePaySystem = this.getSelectedPaySystem();
			var activeDelivery = this.getSelectedDelivery();

			var deliveryName = 'Не выбрано';
			if(this.deliveryType != ''){
				if(this.deliveryType == 'self'){
					deliveryName = activeDelivery.NAME;
					/* if($('#rbs-edost-select-point option:checked').length){
						deliveryName = 'Самовывоз: ' + $('#rbs-edost-select-point option:checked').text();
					} else if($('.rbs-self-address').length){
						deliveryName = 'Самовывоз: ' + $('.rbs-self-address').text();
					} */
					if($('.rbs-self-address').length){
						deliveryName = 'Самовывоз: ' + $('.rbs-self-address').text();
					}					
				} else {
					deliveryName = 'Курьер';
			...
			this.totalInfoBlockNode.appendChild(BX.create('DIV', {props:{className:'total__delivery'}, children: [
				BX.create('DIV', {props:{className:'total__heading'}, text: 'Выбранный способ доставки и оплаты'}),
				BX.create('P', {html: `Доставка — <span>${deliveryName}</span.`}),
				BX.create('P', {html: `Оплата — <span>${activePaySystem ? activePaySystem.NAME : ''}</span.`})
			]}));

			this.totalInfoBlockNode.appendChild(BX.create('DIV', {props:{className:'total__info'}, children: [
				BX.create('P', {html: `Скидка на сумму <div>${total.DISCOUNT_PRICE_FORMATED}</div>`}),
				BX.create('P', {html: `Доставка <div>${total.DELIVERY_PRICE_FORMATED}</div>`})
			]}));
			//var classOrderBtn = 'button button_disabled hide';
			this.totalInfoBlockNode.appendChild(BX.create('DIV', {props:{className:'total__btm'}, children: [
				BX.create('DIV', {props:{className:'total__sum'}, html: `Итого: <div>${total.ORDER_TOTAL_PRICE_FORMATED}</div>`})
				/* BX.create('DIV', {props:{className:'total__btn'}, children: [
					BX.create('BUTTON', {props:{className: classOrderBtn}, text: this.params.MESS_ORDER, events: {click: BX.proxy(this.clickOrderSaveAction, this)}})
				]}) */
			]}));
			
			/*RBS_CUSTOM_START*/
			BX.onCustomEvent('onAfterTotalBlockEdit');
			/*RBS_CUSTOM_END*/
		...
		totalBlockScrollCheck: function()
		{
			...
			if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				...
				this.totalGhostBlockNode.style.paddingTop = '0px';
