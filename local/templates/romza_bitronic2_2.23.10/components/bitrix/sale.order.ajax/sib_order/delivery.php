...
<?
//echo $arResult['USER_VALS']['DELIVERY_ID'];
...
<? if (!empty($arResult['edost']['locations_installed'])) { ?>
    <style>
        ...
        if($bSibCore){
            $isMskRegion = \Sib\Core\Catalog::isMskRegion($_SESSION["VREGIONS_REGION"]['ID']);
            if ($isMskRegion){
                $address_tariff = array(
                    '348' => 'магазин: г. Москва, Багратионовский проезд 7 корпус 3 ТК Горбушкин Двор пав. h2-003',
                    '422' => 'магазин: г. Москва, Багратионовский проезд 7 корпус 3 ТК Горбушкин Двор пав. h2-003',
                );
            } else {
                $address_tariff = array(
                    '348' => 'магазин: г. Новосибирск, ул. Новогодняя, д. 17',
                    '422' => 'магазин: г. Новосибирск, ул. Новогодняя, д. 17',
                );
            }
        }
...
<div class="order-step__heading">                                    
    <div class="order-step__head"><span>2</span>Выберите удобный способ получения</div>
...
<div class="order-step__cont">
    <div class="order-delivery">
        ...
        <?if (!empty($arResult['edost']['locations_installed'])):?>
            <div class="order-delivery-town">
                Доставка в 
                <div class="city city_delivery js-click">
                    <a class="city__selected js-click-button-city" href="javascript:void(0);">
                        <span class="js-city-selected">
                            <? $GLOBALS['APPLICATION']->IncludeComponent('edost:locations', 'sib_loc', array('MODE' => 'city') + $edost_locations_param, null, array('HIDE_ICONS' => 'Y')); ?>
                        </span>
                    </a>
                    ...
                    <? 
                        ...
                        //$APPLICATION->IncludeComponent("bitrix:main.include", "", array_merge($arDefIncludeParams, array("PATH" => SITE_DIR . "include_areas/sib/header/vregions_hide.php")), false, array("HIDE_ICONS" => "Y"));
                    ?>
                </div>
            </div>
        <?endif?>
        <?
            if($_SESSION['TYPE_DELIVERY'] == 'self' || empty($_SESSION['TYPE_DELIVERY'])){
                $address_hide = true;
            }        
        ?>
        <div class="rbs-delivery-type">
            <div data-delivery-type-btn="self" class="rbs-delivery-type__type <?=$_SESSION['TYPE_DELIVERY'] == 'self' ? 'rbs-delivery-type__active' :''?>">
                <input class="hide" type="radio" id="delivery_type_self" name="delivery_type" value="self">
                <div class="rbs-delivery-type__head">Самовывоз</div>
                <div class="rbs-delivery-type__descr">В удобном месте</div>
            </div>
            <div data-delivery-type-btn="courier" class="rbs-delivery-type__type <?=$_SESSION['TYPE_DELIVERY'] == 'courier' ? 'rbs-delivery-type__active' :''?>">
                <input class="hide" type="radio" id="delivery_type_courier" name="delivery_type" value="courier">
                <div class="rbs-delivery-type__head">Курьер</div>
                <div class="rbs-delivery-type__descr">Домой или в офис</div>
            </div>
        </div>
        
        <div class="delivery-options arrows-2 js-delivery-options">
        <?  if(!empty($arResult["DELIVERY"]) && \Bitrix\Main\Loader::includeModule('sib.core'))
            {
                foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
                {
                    if(in_array($delivery_id,array(414))) continue;
                    /* if(!empty($_SESSION['TYPE_DELIVERY'])){
                        if($_SESSION['TYPE_DELIVERY'] != \Sib\Core\Sale::getDeliveryType($arDelivery["ID"])){
                            continue;
                        }
                    } */
        ...
        ?>
                                ...
                                <div class="delivery-options__item" onclick="<?=$onclick?>">
                                    <input
                                            type="radio"
                                            id="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>"
                                            name="<?=htmlspecialcharsbx($arProfile["FIELD_NAME"])?>"
                                        ...
                                        <?=$arProfile["CHECKED"] == "Y" ? "checked=\"checked\"" : "";?>
                                            onclick="submitForm();"
                                    />
                                    <div class="delivery-options__cont js-delivery-choice" onclick="<?=$onclick?>">
                                        <div class="delivery-options__img">
                                            <img src="<?=$deliveryImgURL?>" alt="<?=$arDelivery["TITLE"].'('.$arProfile["TITLE"].')'?>">
                                            <div class="delivery-options__logo"><img src="img/delivery-comp-1.png" alt=""></div>
                                        </div>
                                        <div class="delivery-options__text">
                                            <div class="delivery-options__title">
                                                <!-- <span>DPD</span> (online classic - до пункта выдачи со страховкой) -->
                                                <?=(($delivery_id == 'edost' ? '' : htmlspecialcharsbx($arDelivery["TITLE"]).' - ') . $arProfile["TITLE"] . ($arProfile['day'] != '' ? ', '.$arProfile['day'] : ''))?>
                                            </div>
                                            <!-- <ul class="delivery-options-list">
                                                <li>1-3 дня</li>
                                                <li>Стоимость: <span>0</span></li>
                                            </ul> -->
                                            <?
                                                if($arProfile["CHECKED"] == "Y" && doubleval($arResult["DELIVERY_PRICE"]) > 0):
                                                    ?>
        ...
        <?
                    ...
                    else // stores and courier
                    {
                        ...
                        if($arDelivery["CHECKED"]=="Y"){
                            $activeChoose['DELIVERY'] = $address_tariff[$arDelivery["ID"]]?str_replace('магазин: ', 'Самовывоз: ', $address_tariff[$arDelivery["ID"]]):$arDelivery["NAME"];
                        }
                        /* $isHide = false;
                        if($_SESSION['TYPE_DELIVERY'] && !\Sib\Core\Sale::checkDelivery($delivery_id, $_SESSION['TYPE_DELIVERY'])){
                            $isHide = true;
                        } */

                        if($arDelivery["ID"] == '340' || $arDelivery['ID'] == '342'){
                            $time = '19:00';
                            if($isMskRegion){
                                $time = '14:00';
                            }
                            $arDelivery["PERIOD_TEXT"] = 'Сегодня при заказе до ' . $time;
                        }

         ?>                         

                        <div data-delivery-type="<?=\Sib\Core\Sale::getDeliveryType($arDelivery["ID"])?>" class="delivery-options__item <?=$arDelivery["CHECKED"]=="Y"?'active':''?>">
                            <div class="delivery-options__cont js-delivery-choice" <?=$clickHandler?> for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>">
                                <input type="radio"
                                    id="ID_DELIVERY_ID_<?=$arDelivery["ID"] ?>"
                                    name="<?=htmlspecialcharsbx($arDelivery["FIELD_NAME"])?>"
                                    value="<?= $arDelivery["ID"] ?>"<?if ($arDelivery["CHECKED"]=="Y") echo " checked";?>
                                ...
                                <div class="delivery-options__img">
                                    <img src="<?=$deliveryImgURL?>" alt="<?=$arDelivery["NAME"]?>">
                                    <!-- <div class="delivery-options__logo"><img src="<?=SITE_TEMPLATE_PATH?>/new_img/delivery-comp-1.png" alt=""></div> -->
                                </div>
                                <div class="delivery-options__text">
                                    <div class="delivery-options__title">
                                        <span><?=$address_tariff[$arDelivery["ID"]]?str_replace('магазин: ', '', $address_tariff[$arDelivery["ID"]]):htmlspecialcharsbx($arDelivery["NAME"])?></span>
                                    </div>
                                    <ul class="delivery-options-list">
                                        <?if(strlen($arDelivery["PERIOD_TEXT"])>0):?>
                                            <li><?=$arDelivery["PERIOD_TEXT"];?></li>
                                        <?endif?>
                                        <li>Стоимость: <span><?=$arDelivery["PRICE_FORMATED"]?><span class="icon-rub"></span></span></li>
                                    </ul>
                                    <?//global $USER; if($USER->IsAdmin()){echo '<pre>'; print_r($arDelivery); echo '</pre>';}; ?>
                                    <?if((int)$arDelivery["PRICE"] === 0):?>
                                        <div class="delivery-options__free">Бесплатно!</div>
                                    <?endif?>
    ...
    ?>               
        ...
        <?
            ...
            if (!empty($arResult['edost']['locations_installed']) && isset($arResult['edost']['order_prop']['ADDRESS'])) {
                ?>
                <div class="delivery-address" style="display:<?=($address_hide ? 'none' : 'block');?>">
                    <div id="edost_location_address_head box-field__input" class="delivery-address__title" style="display:<?=($address_hide ? 'none' : 'block');?>">
                        Адрес доставки:
                    </div>
                    <div class="rbs-need-address">Необходимо ввести адрес доставки</div>
                    <? $GLOBALS['APPLICATION']->IncludeComponent('edost:locations', 'sib_loc', array('MODE' => 'address') + $edost_locations_param, null, array('HIDE_ICONS' => 'Y')); ?>
                </div>
                <?
            }
            /* ==================== edost.locations */
        ?>
        <button class="button button_white js-next-step-order" data-block="delivery">Продолжить</button>
    </div>
</div>