...
if (!empty($arResult["ORDER"])):
	...
	$bSibCore = CModule::IncludeModule('sib.core');
	...
	if($bSibCore){
		if(\Sib\Core\Sale::getDeliveryType($arResult['ORDER']['DELIVERY_ID']) == 'self'){
			if($order = \Bitrix\Sale\Order::loadByAccountNumber($arResult["ORDER"]["ACCOUNT_NUMBER"])){
				if($props = $order->getPropertyCollection()){
					if($prop = $props->getItemByOrderPropertyId(7)){
						if($addr = $prop->getValue()){
							$expl = explode(',', $addr);
						}
					}
				}
			...
			if(is_array($expl) && strpos(end($expl), 'од филиала') !== false){
				array_pop($expl);
				$arResult["DELIVERY"]["NAME"] = implode(',', $expl);
			} else if($addr){
				$arResult["DELIVERY"]["NAME"] = $addr;
			} else {
				$arResult["DELIVERY"]["NAME"] = 'Самовывоз';
			}			
		}
		if(\Sib\Core\Sale::getDeliveryType($arResult['ORDER']['DELIVERY_ID']) == 'courier'){
			$arResult["DELIVERY"]["NAME"] = 'Курьер';
		}
	}
	...
	//global $USER; if($USER->IsAdmin()){echo '<pre>'; print_r($arResult['ORDER']); echo '</pre>';}
?>
<style>
.main-block_order{margin-top:20px;}
.total{margin-left: 19px;}
@media(max-width:992px){
	.total.total_order{margin-left:0;}
}
</style>
<section class="main-block main-block_order" data-page="order-details-page" id="order_form_div" >

	<div class="main-block main-block_order main-block_order-success" <?=$arResult['PAY_SYSTEM']['ID'] == 27 ? 'style="width:100%"' : ''?>>
		<?//global $USER; if(!$USER->IsAdmin()){echo '<pre>'; print_r($_SESSION['VREGIONS_REGION']); echo '</pre>';};?>
		<?		
			if($arResult['PAY_SYSTEM']['ID'] != 27){
				include($_SERVER["DOCUMENT_ROOT"]."/include_areas/sib/order/confirm.php");
			} else {
				include($_SERVER["DOCUMENT_ROOT"]."/include_areas/sib/order/confirm_pay.php");
			}
		?>
		<div class="total total_order">
			...
			<div class="total__delivery">
				...
				<?if($arResult["DELIVERY"]["NAME"]):?>
					<p><?=GetMessage('BITRONIC2_SOA_TEMPL_SUM_DELIVERY');?> <span><?=$arResult["DELIVERY"]["NAME"]?: 'Не выбрано'?></span></p>
				<?endif?>
