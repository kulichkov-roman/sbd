	...
	BX.Sale.OrderAjaxComponent = {
		...
		createDeliveryItem: function(item)
		{
			...
			//Построение блока	
				...
				if(item.DESCRIPTION){
					item.isEdostItem = item.NAME.indexOf('СДЭК') != -1 || item.NAME.indexOf('DPD') != -1;
				}
			...
			modifyCustomItem(item)
			{
				...
				if(item.ID == 340){
					item.day = `<div style="white-space:normal">Сегодня при заказе до 19:00</div>`;
					if(this.params.CITY_NAME == 'Москва'){
						item.day = `<div style="white-space:normal">Сегодня при заказе до 13:00</div>`;
					}
				}
