...
<main class="container order-details-page" data-page="order-details-page" id="order_form_div">
...
?>
	...
	else
	{
		...
		else
		{
			...
			{
				?>
				<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="ORDER_FORM" enctype="multipart/form-data" class="form-order">
				<?=bitrix_sessid_post()?>
			...
			<?if($_POST["is_ajax_post"] != "Y")
			{
				?>
					...
					<div class="make-order-wrap">
						<button class="make-order" onclick="submitForm('Y'); return false;" id="ORDER_CONFIRM_BUTTON"><span class="btn-text"><?=GetMessage("BITRONIC2_SOA_TEMPL_BUTTON")?></span></button>
					</div>
				</form>
				<?
