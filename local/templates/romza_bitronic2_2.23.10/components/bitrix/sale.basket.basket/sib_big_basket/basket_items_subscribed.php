		...
		<thead>
			<tr>
				...
				foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
					...
					if ($arHeader["name"] == '')
						$arHeader["name"] = GetMessage("BITRONIC2_SALE_".$arHeader["id"]);
		...
		<tbody>
			...
			?>
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						?>
							<td class="itemphoto">
								...
								<div itemscope itemtype="http://schema.org/ImageObject" class="bx_ordercart_brand">
									<?
									if (!empty($arItem["BRAND"])):
									?>
										<img itemprop="contentUrl" alt="" class="lazy" data-original="<?=$arItem["BRAND"]?>" src="<?=ConsVar::showLoaderWithTemplatePath()?>"/>
									<?
									endif;
									?>
								</div>
						...
						?>
							<td class="price">
								...
								<?if (strlen($arItem["NOTES"]) > 0):?>
									<div class="type_price"><?=GetMessage("BITRONIC2_SALE_TYPE")?></div>
					...
					?>
						<td class="control">
							<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><?=GetMessage("BITRONIC2_SALE_DELETE")?></a><br />
						</td>
						<td class="margin"></td>
				</tr>
				<?
