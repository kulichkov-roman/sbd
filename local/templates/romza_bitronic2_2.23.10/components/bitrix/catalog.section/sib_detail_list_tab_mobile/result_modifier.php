...
foreach ($arResult['ITEMS'] as $index => $arItem)
{
	...
	$sectionNames[$arItem['PROPERTIES'][$propSection]['VALUE_ENUM_ID']] = $arItem['PROPERTIES'][$propSection]['VALUE'];
	$iblockSections[$arItem['PROPERTIES'][$propSection]['VALUE_ENUM_ID']][] = $arItem['ID'];
	$arItemBySection[$arItem['PROPERTIES'][$propSection]['VALUE_ENUM_ID']][] = $arItem;
}

$arResult['ITEMS_SECTION'] = $arItemBySection;
...
if (!function_exists('findParentSection')) {
    function findParentSection($sectionId = false, $iblockId = false)
    {
        if (!$sectionId || !$iblockId) {
            return false;
        }        
		
		$rsSect = CIBlockSection::GetList([], ['IBLOCK_ID' => $iblockId, 'ID' => $sectionId], false, ['ID', 'NAME', 'IBLOCK_SECTION_ID', 'DEPTH_LEVEL']);
        if ($arSect = $rsSect->GetNext()) {
            if ((int)$arSect['DEPTH_LEVEL'] <= 2) {
                return $arSect;
            }
            if ((int)$arSect['IBLOCK_SECTION_ID'] > 0) {
                return findParentSection($arSect['IBLOCK_SECTION_ID'], $iblockId);
            }
        } else {
            return false;
        }
...
$arResult['SECTIONS'] = [];
if(count($iblockSections) > 0) {
	foreach($iblockSections as $sectionId => $items){
		$arResult['SECTIONS'][$sectionId]['NAME'] = $sectionNames[$sectionId];
		$arResult['SECTIONS'][$sectionId]['COUNT'] = count($items);
	}
}
...
$arSortParams = [
	[
		...
		'BY' => $arParams['ELEMENT_SORT_ORDER2'],
		'NAME' => 'По цене'
	],
	[
		'SORT' => 'SHOWS',
...
$arResult['SORT_PARAMS'] = $arSortParams;