...
if (!empty($arResult['ITEMS']))
{
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem)
    {
        ...
        $arItem['PICTURE_PRINT']['SRC'] = CRZBitronic2CatalogUtils::getElementPictureById($arItem['ID'], 43);
...
//SORT
$arNewItemSort = [];
foreach($arParams['ITEMS'] as $itemId){
    foreach($arResult['ITEMS'] as $arItem){
        if($arItem['ID'] == $itemId){
            $arNewItemSort[] = $arItem;
            continue;
        }
    }
}
$arResult['ITEMS'] = $arNewItemSort;