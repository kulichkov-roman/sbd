...
if (!empty($arResult['ITEMS']))
{
    ...
    $ob = new \CPHPCache();
    $cacheString = md5(SITE_ID.'sections_in_brand_page'.implode(';',$arSectionsID));
    $cachePath = '/' . SITE_ID . '/SectionsForBrands/'.implode('.',$arSectionsID);

    if ($ob->InitCache(360000, $cacheString, $cachePath)) {
        $arResult['SECTIONS_WITH_ELEMENTS'] = $ob->GetVars();
        ...
        $ob->EndDataCache($arResult['SECTIONS_WITH_ELEMENTS']);
    }
