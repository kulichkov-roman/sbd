...
if($arParams['IS_SMARTPHONE_ITEM'] === 'Y' && count($hydroEls) > 0){
	$arResult['CUSTOM_ITEMS'][] = [
		...
		'PIC' => [
			'WEBP' => CResizer2Resize::ResizeGD2(SITE_TEMPLATE_PATH . '/img/hydrogel.jpg', 3),
			'JPG' => CResizer2Resize::ResizeGD2(SITE_TEMPLATE_PATH . '/img/hydrogel.jpg', 43),
		],
		'STICKERS' => [
			'VALUE' => [
...
foreach ($arResult['ITEMS'] as $index => $arItem)
{
	...
	if ($arParams['HIDE_ICON_SLIDER'] != 'Y' && !$arParams['IS_MOBILE']) {
		...
		$arItem['PICTURE_PRINT']['SRC'] = CResizer2Resize::ResizeGD2($arItem['MORE_PHOTO'][0]['SRC'], 3);
    ...
    if ($arBasketItem = $dbBasketItems->Fetch())
        $arResult['ITEMS'][$index]['IN_BASKET'] = $arBasketItem['ID'];
}
