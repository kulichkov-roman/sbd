	...
	foreach ($arParams["LOC_DEFAULT"] as $val):
		...
		<label class="radio-styled">
			...
			<span class="radio-content">
				...
				<span class="text"><?=$val["LOC_DEFAULT_NAME"]?></span>
			</span>
		</label>
	<?endforeach;?>
	<label class="radio-styled">
		...
		<span class="radio-content">
			<span class="radio-fake"></span>
			<span class="text"><?=GetMessage("LOC_DEFAULT_NAME_NULL")?></span>
		</span>
	</label>
<?endif;?>
...
<?if (count($arResult["COUNTRY_LIST"]) > 0):?>
	...
	<?endif?>
	<select <?if($disabled || $cDisabled) echo "disabled";?> id="<?=$arParams["COUNTRY_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>" name="<?=$arParams["COUNTRY_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>" onChange="<?=$change?>" type="location" class="select-styled">
...
<?if (count($arResult["REGION_LIST"]) > 0):?>
	...
	<?if($rDisabled):?>
		<div style="display:none">
	<?endif?>
	<select <?=$id?> <?if($disabled || $rDisabled) echo "disabled";?> name="<?=$arParams["REGION_INPUT_NAME"].$arParams["CITY_INPUT_NAME"]?>" onChange="<?=$change?>" type="location" class="select-styled">
...
<?if (count($arResult["CITY_LIST"]) > 0):?>
	...
	<?if($cDisabled):?>
		<div style="display:none">
	<?endif?>
	<select <?=$id?> <?if($disabled) echo "disabled";?> name="<?=$arParams["CITY_INPUT_NAME"]?>"<?if (strlen($arParams["ONCITYCHANGE"]) > 0):?> onchange="<?=$arParams["ONCITYCHANGE"]?>"<?endif;?> type="location" class="select-styled">
