...
else
{
	...
	foreach($arResult['TABS'] as $codeTab => &$arTab)
	{
		...
		$arTab['HEADER'] = GetMessage('MAINSPEC_TAB_' . $codeTab);
        $arTab['ALL'] = GetMessage('MAINSPEC_ALL_' . $codeTab);
		...
		if ($arProp && isset($arProps[$arProp['ID']]) && $arProps[$arProp['ID']]['SMART_FILTER'] == 'Y') {
			...
			$arTab['LINK'] = '/catalog/smartfony/?arrFilter' . $arProp['VALUES'][$value]['CONTROL_ID'] . '=Y&amp;set_filter=y&amp;rz_all_elements=y';
		} else {
			$arTab['LINK'] = false;
		}
	}
