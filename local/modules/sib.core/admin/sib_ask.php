...
$bSibCore = Loader::includeModule("sib.core");
...
$aContext[] = array(
    "TEXT" => 'Новые',
	"LINK" => "sib_ask.php?IBLOCK_ID=".$_GET['IBLOCK_ID']."&TYPE=".$_GET['TYPE']."&ACTIVE=N&type=references&lang=".LANGUAGE_ID,
);

$aContext[] = array(
    "TEXT" => 'Активные',
    "LINK" => "sib_ask.php?IBLOCK_ID=".$_GET['IBLOCK_ID']."&TYPE=".$_GET['TYPE']."&ACTIVE=Y&type=references&lang=".LANGUAGE_ID,
);
/*RBS_CUSTOM_END*/
...
?>
<style>
#find_form, .adm-list-table-popup-block{display:none;}
</style>
<?if($_GET['TYPE'] == \Sib\Core\Ask::getTypeAskId()):?>
<style>
.adm-list-table-popup-block{display:table-cell;}
</style>
<div id="ajax-add-answer">
	<div class="message-error msg" style="display:none;">
	...
	<div class="message-ok msg" style="display:none;">
		<div class="adm-info-message-wrap adm-info-message-green">
			<div class="adm-info-message">
				<div class="adm-info-message-title">Ответ добавлен!</div>
				<div class="msg-text"></div>
				<div class="adm-info-message-icon"></div>
			</div>
		</div>
	</div>
    <form action="/ajax/sib/ask.php" id="formAns">
        <input type="hidden" name="askId" value=""/>
        <input type="hidden" name="type" value="addAnsUser"/>
		<input type="hidden" name="userId" value="<?=$USER->GetID()?>"/>
		<input type="hidden" name="userName" value="<?=$USER->GetFullName()?>"/>
        <textarea name="ans" id="" cols="60" rows="20"></textarea>
    </form>
</div>
<script>
    function addAnswerAction(id){
		$('#formAns [name="askId"]').val(id);
		clearMsg();
        addAnswer.show();
	}
	
	function clearMsg(){
		$('#ajax-add-answer .msg').hide();
	}

    BX.ready(function(){
        addAnswer = new BX.PopupWindow("answer_id", null, {
            content: BX('ajax-add-answer'),
            titleBar: {content: BX.create("h3", {html: '<b>Ответить</b>', 'props': {'className': 'access-title-bar'}})},
            zIndex: 9,
            offsetLeft: 0,
            offsetTop: 0,
                    ...
                    events: {click: function(){
                        BX.ajax.post(
                            ...
                            function (data) {
								...
								clearMsg();
								if(data.TYPE == 'ERROR'){
									$('#ajax-add-answer .message-error').show().find('.msg-text').html(data.MSG);
								}
								if(data.TYPE == 'OK'){
									$('#ajax-add-answer .message-ok').show().find('.msg-text').html(data.MSG);
									$('#formAns textarea').val('');
									$('.ask-id-'+$('#formAns [name="askId"]').val()).closest('tr').find('td').css({'background':'green'});
								}
                            }
                        );
                    }}
        ...
        var $cnt = 0;
        $('.adm-list-table-popup-block').each(function(){
            if($cnt++ > 0){
                var elId = +$(this).siblings('td').last().find('a').html();
                $(this).html('<a href="javascript:;" class="adm-btn ask-id-'+elId+'" onclick="addAnswerAction('+elId+')">Ответить</a>');
                $(this).attr('onclick', '');
            }
        });
});
