...
header("Content-Type: text/plain");?>'.PHP_EOL.$robots_file;
    ...
    function generate_robots_difficult($prop, $makePhpFile){
        $answer            = Array();
        $answer["success"] = false;
...
echo str_replace("<br>", "", html_entity_decode($_SESSION["VREGIONS_REGION"]["'.$prop.'"]["TEXT"]));?>';
    ...
    public static function generateRegionLink($code, $protocol = '', $regionFullUrlProp = false, $www = false){
        ...
        /* define("LOG_FILENAME", $_SERVER["DOCUMENT_ROOT"]."/local/modules/aristov.vregions/log_update_link.txt");
        AddMessage2Log($href); */
        return $href;
    }
    ...
    public static function setModuleOption($optionCode, $value){
    ...
    public static function getModuleOption($optionCode, $default = ''){
        if (!class_exists('CMainPage')){
            include_once($_SERVER['DOCUMENT_ROOT']."/bitrix/modules/main/include/mainpage.php");
        }
       ...
       // $siteID = \CMainPage::GetSiteByHost();
        $siteID = 's1';
        return \COption::GetOptionString(static::$moduleID, $optionCode, $default, $siteID);
    }
