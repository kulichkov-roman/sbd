...
if(\Bitrix\Main\Loader::includeModule('sib.core'))
{
    ...
    $arParams['TAB_PROPERTY_NEW'] = \Sib\Core\Catalog::getTabPropertyNew();
...
if($_REQUEST['q']){
    $_GET['q'] = $_REQUEST['q'];
    if($_REQUEST['where']) $_GET['where'] = $_REQUEST['where'];
    if($_REQUEST['section']) $_GET['section'] = $_REQUEST['section']; 
}

if($_REQUEST['CUSTOM_CACHE_KEY']){
    if($_REQUEST['section']) $_GET['section'] = $_REQUEST['section'];
}
