    ...
    case 'updateBasket':
        ...
        if (\Bitrix\Main\Loader::includeModule('sib.core')){
            ...
            if(count($basketItems) > 0){
                ...
                foreach ($basketItems as $basketItem) {
                    ...
                    if($priceArray['PRICE_DISCOUNT'] > 0){
                        $basketItem->setFields([
                            'PRICE' => $priceArray['PRICE_DISCOUNT'],
                            'CUSTOM_PRICE' => 'Y'
                        ]);
                    }

                    $itemInf = \Sib\Core\Helper::getItemInf($pid);
                    if($itemInf['IS_SMARTPHONE'] && $basketItem->canBuy()){
                        $addedSmartPhones[$pid] = $itemInf['NAME'];
                        continue;
                    }

                    $basketPropertyCollection = $basketItem->getPropertyCollection(); 
                    foreach($basketPropertyCollection->getPropertyValues() as $code => $prop){
                        if($prop['CODE'] == 'FOR_PID'){
                            $addedServices[$prop['VALUE']][] = $basketItem->getId();
                        }
                    }
                }
                ...
                foreach($addedSmartPhones as $smartphoneId => $name){
                    if(isset($addedServices[$smartphoneId])) continue;
                    \Sib\Core\Helper::addFreeServices($name, $smartphoneId);
                }

                foreach($addedServices as $smartphoneId => $services){
                    if(!isset($addedSmartPhones[$smartphoneId])){
                        \Sib\Core\Helper::deleteFreeServices($services);
                    }
                }
            }            
        }
